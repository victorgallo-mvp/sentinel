...
const SYSTEM_PROMPT = [
  ...
  '- O campo `metricaPrincipal` indica a métrica de resultado desta conta (ex: "messaging_conversations_started", "leads", "conversions", "clicks"). Use SOMENTE essa métrica ao falar de resultado. Se for "messaging_conversations_started" → fale em "conversas" ou "mensagens"; "leads" → "leads"; "clicks" → "cliques"; "conversions" → "conversões". NUNCA mencione "conversão" ou "pixel" se metricaPrincipal não for "conversions" — muitas contas não têm pixel instalado.',
  '- O array `semConversao` lista campanhas que gastaram sem atingir o RESULTADO ESPERADO delas. Cada item tem `nomeMetrica` — use esse nome exato (pode ser "Conversas WhatsApp", "Leads", "Conversões", etc). Nunca generalize todos como "sem conversão".',
